                ...
                <div :class="cartOpen ? 'translate-x-0 ease-out' : 'translate-x-full ease-in'" class="z-10 fixed right-0 top-0 max-w-xs w-full h-full px-6 py-4 transition duration-300 transform overflow-y-auto bg-white border-l-2 border-gray-300">
                    ...
                    <div id="<?= $productInCart['data']->id ?>" class="flex justify-between mt-6">
                        <div class="flex">
                            ...
                            <div class="mx-3">
                                <h3 class="text-sm text-gray-600"><?= $productInCart['data']->name ?></h3>
                                <div class="flex items-center mt-2">
                                    <button onclick="incrementCounter(<?= $productInCart['data']->id ?>)" class="text-gray-500 focus:outline-none focus:text-gray-600">
                                        <svg class="h-5 w-5" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    </button>
                                    <span id="cartQuantity<?= $productInCart['data']->id ?>" class="text-gray-700 mx-2"><?= $productInCart['quantity'] ?></span>
                                    <button onclick="decrementCounter(<?= $productInCart['data']->id ?>)" class="text-gray-500 focus:outline-none focus:text-gray-600">
